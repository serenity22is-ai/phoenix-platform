/**
 * MYSTES iOS Background Tasks (Build #91)
 *
...
let HEARTBEAT_TASK_ID = "com.mystes.app.nodeHeartbeat"
let FLUSH_TASK_ID = "com.mystes.app.nodeFlush"

// MARK: - MystesBackgroundTasks

class MystesBackgroundTasks {
    ...
    static func registerBackgroundTasks() {
        ...
        print("[MystesBG] Background tasks registered")
    }
    ...
    static func scheduleHeartbeat() {
        ...
        do {
            ...
            print("[MystesBG] Heartbeat task scheduled")
        } catch {
            print("[MystesBG] Failed to schedule heartbeat: \(error)")
        }
    }
    ...
    static func scheduleFlush() {
        ...
        do {
            try BGTaskScheduler.shared.submit(request)
            print("[MystesBG] Flush task scheduled")
        } catch {
            print("[MystesBG] Failed to schedule flush: \(error)")
        }
    }
    ...
    private static func handleHeartbeatTask(task: BGAppRefreshTask) {
        ...
        guard !serverURL.isEmpty, !helperToken.isEmpty, !nodeId.isEmpty else {
            print("[MystesBG] Missing config — skipping heartbeat")
        ...
        task.expirationHandler = {
            print("[MystesBG] Heartbeat task expired")
        }
        ...
        request.setValue("MystesNodeClient/1.0.0 (ios-bg)", forHTTPHeaderField: "User-Agent")
        ...
        let dataTask = session.dataTask(with: request) { data, response, error in
            if let error = error {
                print("[MystesBG] Heartbeat failed: \(error.localizedDescription)")
                task.setTaskCompleted(success: false)
                return
            }

            if let httpResponse = response as? HTTPURLResponse {
                print("[MystesBG] Heartbeat response: \(httpResponse.statusCode)")
    ...
    private static func handleFlushTask(task: BGProcessingTask) {
        ...
        print("[MystesBG] Flush task running (delegated to WebView)")
    ...
    static func configure(serverURL: String, helperToken: String, nodeId: String) {
        ...
        self.nodeId = nodeId
        print("[MystesBG] Configured: server=\(serverURL), node=\(nodeId)")
    }
}
 ...
 *    <array>
 *        <string>com.mystes.app.nodeHeartbeat</string>
 *        <string>com.mystes.app.nodeFlush</string>
 *    </array>
