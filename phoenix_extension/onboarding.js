/**
 * Phoenix Node Extension — Onboarding Wizard (Build #83)
 *
...
const DEFAULT_SERVER_URL = "https://phoenix.example.com";
...
async function signInWithGoogle() {
  ...
  try {
    ...
    console.log("[Phoenix] Got Google access token");
    ...
    console.log("[Phoenix] Google OAuth success:", result.userName);

    await handleOAuthSuccess(result, serverUrl);
  } catch (error) {
    console.error("[Phoenix] Google sign-in error:", error);
    showAuthButtons();
...
async function signInWithMicrosoft() {
  ...
  try {
    ...
    console.log("[Phoenix] Got Microsoft access token");

    // Exchange with Phoenix backend
    const serverUrl = DEFAULT_SERVER_URL;
    ...
    const result = await response.json();
    console.log("[Phoenix] Microsoft OAuth success:", result.userName);

    await handleOAuthSuccess(result, serverUrl);
  } catch (error) {
    console.error("[Phoenix] Microsoft sign-in error:", error);
    showAuthButtons();
    loadingState.classList.remove("visible");
    authError.textContent = error.message;
    authError.classList.add("visible");
...
async function signInWithApple() {
  ...
  try {
    ...
    console.log("[Phoenix] Got Apple id_token");
    ...
    const result = await response.json();
    console.log("[Phoenix] Apple OAuth success:", result.userName);

    await handleOAuthSuccess(result, serverUrl);
  } catch (error) {
    console.error("[Phoenix] Apple sign-in error:", error);
    showAuthButtons();
    loadingState.classList.remove("visible");
    authError.textContent = error.message;
    authError.classList.add("visible");
...
async function signInWithManualToken() {
  ...
  try {
    ...
    console.error("[Phoenix] Manual auth error:", error);
    authError.textContent = error.message;
    authError.classList.add("visible");
  }
}
...
async function finishOnboarding() {
  ...
  if (authCredentials && authCredentials.helperToken) {
    try {
      ...
      console.log("[Phoenix] Backend consent push deferred:", e.message);
    }
  }
  ...
  if (authCredentials && authCredentials.userName && authCredentials.userName !== "Node Operator") {
    successMsg.innerHTML =
      `Welcome, <strong>${authCredentials.userName}</strong>! Phoenix Node is ready.<br>` +
...
console.log("[Phoenix] Onboarding wizard loaded (Build #83)");
