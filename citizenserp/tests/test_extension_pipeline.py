...
# All extension pipeline tests require Phase 2 P2P routes (extracted Build #136)
def _has_route(rule: str) -> bool:
    with app.app_context():
        return any(r.rule == rule for r in app.url_map.iter_rules())

_phase2_available = _has_route("/api/node/auth")
pytestmark = pytest.mark.skipif(not _phase2_available, reason="Phase 2 node routes not registered (Build #136)")
