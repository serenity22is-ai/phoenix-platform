/**
 * MYSTES Mobile Node Client (Build #91)
 *
 * JavaScript port of node_service.py core logic for Capacitor WebView.
 * Talks directly to the Mystes backend REST API (not localhost:19750).
 * Manages authentication, heartbeat, event buffering, and batch upload.
 *
 * Usage:
 *   const client = new MystesNodeClient(serverUrl, helperToken);
 *   await client.start();
...
(function (root) {
  ...
  class MystesNodeClient {
    /**
     * @param {string} serverUrl  Mystes backend base URL
    ...
    async start() {
      ...
      console.log("[MystesNode] Starting node client, server:", this.serverUrl);

      const authenticated = await this._authenticate();
      if (!authenticated) {
        console.warn("[MystesNode] Auth failed — will retry in background");
    ...
    async stop() {
      if (!this._running) return;
      console.log("[MystesNode] Stopping node client...");
      this._running = false;
      this._stopLoops();
    ...
    async _apiCall(method, path, body = null) {
      ...
      const headers = {
        ...
        "User-Agent": `MystesNodeClient/${VERSION} (${this.nodeType})`,
      };
    ...
    async _authenticate() {
      try {
        ...
        if (resp.ok) {
          ...
          this._reconnectDelay = RECONNECT_BASE_DELAY_MS;
          console.log("[MystesNode] Authenticated — node_id:", this.nodeId);
          return true;
        } else {
          console.warn("[MystesNode] Auth failed:", resp.status);
          return false;
        }
      } catch (err) {
        console.warn("[MystesNode] Auth error:", err.message);
        return false;
      }
    }
    ...
    async _startSession() {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          console.log("[MystesNode] Session started:", this.sessionId);
        } else {
          console.warn("[MystesNode] Session start failed:", resp.status);
        }
      } catch (err) {
        console.warn("[MystesNode] Session start error:", err.message);
      }
    }
    ...
    async _endSession() {
      try {
        ...
        console.log("[MystesNode] Session ended");
      } catch (err) {
        console.warn("[MystesNode] Session end error:", err.message);
      } finally {
        this.sessionId = null;
        this._connected = false;
      }
    ...
    async _sendHeartbeat() {
      ...
      try {
        ...
        console.warn("[MystesNode] Heartbeat error:", err.message);
        if (!this._connected) {
          this._scheduleReconnect();
        }
      }
    ...
    async _flushBuffer() {
      ...
      try {
        ...
        if (resp.ok) {
          ...
          console.log(
            `[MystesNode] Uploaded ${batch.length} events (accepted=${accepted}, value=$${valueUsd.toFixed(6)})`
          );
          if (this.onEarnings && valueUsd > 0) {
            this.onEarnings(valueUsd);
          }
        } else {
          console.warn("[MystesNode] Upload failed:", resp.status);
          this._uploadErrors++;
        ...
        console.warn("[MystesNode] Upload error:", err.message);
        this._uploadErrors++;
        this._eventBuffer.unshift(...batch);
        this._trimBuffer();
      }
    ...
    async _fetchConfig() {
      try {
        ...
        console.debug("[MystesNode] Config fetch error:", err.message);
      }
    }
    ...
    _scheduleReconnect() {
      ...
      const attempt = async () => {
        ...
        console.log(`[MystesNode] Reconnecting in ${this._reconnectDelay / 1000}s...`);
        ...
        if (ok) {
          ...
          this._reconnecting = false;
          this._emitStatus();
          console.log("[MystesNode] Reconnected successfully");
        } else {
    ...
    _trimBuffer() {
      if (this._eventBuffer.length > MAX_BUFFER_SIZE) {
        ...
        console.warn(`[MystesNode] Buffer overflow, dropped ${dropped} oldest events`);
      }
    }
    ...
    _logSummary() {
      ...
      console.log("[MystesNode] === Session Summary ===");
  ...
  root.MystesNodeClient = MystesNodeClient;
