...
engine decides which tools to invoke, executes them against real Phoenix modules,
...
Economic Model (three-phase):
    ...
    Phase 3 (own GDS): Phoenix owns SERP/GDS. Zero external API costs.
    ...
    Set PHOENIX_ECONOMIC_PHASE=2 or =3 in env to activate.

Usage:
    from phoenix_ai import phoenix_ai, get_ai_tier, check_ai_quota

    result = phoenix_ai.chat(
        user_id=42,
...
#
# Phase 3 (own GDS): Phoenix is its own accredited GDS/SERP service.
#   Zero external query costs (we own the SERP). All tiers near-unlimited.
#   Dynamic arbitrage rewards: nodes earn % of Phoenix's booking fee cut
...
# Flip this to 3 when Phoenix is accredited and owns its own GDS/SERP.
# ---------------------------------------------------------------------------
PHOENIX_ECONOMIC_PHASE = int(os.environ.get("PHOENIX_ECONOMIC_PHASE", "1"))
...
# Every query costs Phoenix ~$0.005 in Amadeus fees.
...
# ---------------------------------------------------------------------------
# Phase 3: Own GDS/SERP — Phoenix is accredited, owns the search service
# Zero external API costs. Queries are free to Phoenix (our own SERP network).
...
# Dynamic arbitrage rewards: nodes earn a cut of Phoenix's 25% booking fee
...
if PHOENIX_ECONOMIC_PHASE >= 3:
    ARBITRAGE_FREE_QUERIES = PHASE3_NODE_QUERIES
elif PHOENIX_ECONOMIC_PHASE >= 2:
...
PHOENIX_AI_SYSTEM_PROMPT = (
    "You are Phoenix AI, the intelligent assistant for the Phoenix platform — "
    ...
    "When a user asks to search for flights and provides an origin, destination, and "
    "at least one date, IMMEDIATELY call the search_flights tool. Do NOT ask clarifying "
    "questions first. Use these defaults for anything not specified:\n"
    "- Trip type: one-way (unless they mention a return date)\n"
    "- Cabin class: economy\n"
    "- Passengers: 1\n"
    "The user wants results fast. Search first, then offer to refine.\n\n"

    "You have access to powerful tools that connect to live Phoenix data:\n"
    "- search_flights: Search flights with Phoenix's real-time price comparison engine\n"
    "- search_hotels, search_cruises, search_rentals: Cross-market travel search\n"
...
PHOENIX_AI_TOOLS = [
    ...
    },
    {
        "name": "search_hotels",
        "description": (
            "Search for hotels in a location. Uses the Phoenix agent to find "
            "hotel options across geographic markets for price comparison."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "location": {
                    "type": "string",
                    "description": "City or location to search hotels in (e.g. 'Tokyo', 'Paris')",
                },
                "checkin_date": {
                    "type": "string",
                ...
                "market": {
                    "type": "string",
                    "description": "Geographic market code for price comparison (e.g. 'JP', 'US')",
                },
            },
    ...
    },
    {
        "name": "get_trending",
        "description": (
            "Get trending data on the Phoenix platform — new high-savings deals, "
    ...
    },
    {
        "name": "get_deals",
        "description": (
            "Get the latest arbitrage deals found by Phoenix — flights with "
    ...
    },
    {
        ...
        "description": (
            "Run the Phoenix agent's autonomous opportunity discovery — finds "
    ...
    },
    {
        ...
        "description": (
            "Get the Phoenix node network coverage map — zone hierarchy, node "
            "counts, density scores, and observation volumes. Shows where "
            "Phoenix has real-browser pricing intelligence."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    # Build #80 — Strategy deployment tool
    {
        ...
        "description": (
            ...
            "underserved markets that Phoenix should monitor more closely. "
...
class PhoenixAI:
    """
    LLM-powered tool-calling engine for the Phoenix platform.

    Connects Claude (via Anthropic Messages API) to 14 Phoenix capabilities
    through structured tool definitions. Maintains multi-turn conversation
    context via the AIMessage database table.
    """

    def __init__(self):
        self.tools = PHOENIX_AI_TOOLS
        self.system_prompt = PHOENIX_AI_SYSTEM_PROMPT
        self.logger = logging.getLogger(f"{__name__}.PhoenixAI")
    ...
    def chat(self, user_id, conversation_id, user_message, tier_config):
        """
        Process a user message through the Phoenix AI engine.
    ...
    def _execute_tool(self, tool_name, tool_input, user_id):
        ...
        Dispatches to the appropriate Phoenix module. Each branch is wrapped
        ...
        try:
            if tool_name == "search_flights":
                ...
                market = tool_input.get("market", "US")
                return phoenix_agent.handle_search(
                    query=query,
                ...
                destination = tool_input.get("destination", "").upper()
                market = tool_input.get("market", "US")
                return phoenix_agent.analyze_route(
                    origin=origin,
                    destination=destination,
                    user_market=market,
                )
                ...
                from phoenix_agent import phoenix_agent
                return phoenix_agent.discover_opportunities()
    ...
    def _format_tool_result(self, tool_name, result):
        ...
        try:
            if tool_name == "search_flights":
                ...
                return self._format_arbitrage_search(result, "Hotel")
            elif tool_name == "search_products":
        ...
        if flights:
            ...
            for i, f in enumerate(flights[:10], 1):
                ...
                if deal and deal.get("price_difference", 0) > 0:
                    savings_str = f" (save ${deal['price_difference']:.0f} via Phoenix)"
            ...
            if proxy and proxy.get("savings_vs_us", 0) > 0:
                lines.append(f"\nPhoenix Price Advantage:")
                lines.append(f"  Phoenix price: ${proxy.get('cheapest_price_usd', 'N/A'):.0f}")
    ...
    def _format_agent_search(self, result, vertical):
        """Format phoenix_agent search results."""
        ...
        if deals:
            ...
            for i, deal in enumerate(deals[:5]):
                ...
                lines.append(
                    ...
                    f"(save ${savings:.0f}, {pct:.0f}% off via Phoenix)"
                )
        else:
    ...
    def _format_route_analysis(self, result):
        ...
        if intel and isinstance(intel, dict) and "error" not in intel:
            ...
            if markets:
                cheapest = markets[0]
                lines.append(
                    f"  Phoenix price: ${cheapest.get('avg_price_usd', 'N/A')} avg"
                )
    ...
    def _format_route_intelligence(self, result):
        ...
        if markets:
            ...
            lines.append(f"  Best Phoenix price: ${markets[0].get('min_price_usd', 0):.0f}" if markets else "")
            lines.append(f"  Best booking: Phoenix")
        ...
        if deals:
            ...
            best = deals[0]
            lines.append(
                f"    Best: {best.get('airline', '?')} via Phoenix "
    ...
    def _format_trending(self, result):
        ...
        lines = ["Phoenix Platform Trending:"]
...
def check_ai_quota(user):
    ...
    base = {
        ...
        "economic_phase": PHOENIX_ECONOMIC_PHASE,
