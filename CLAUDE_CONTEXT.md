# MYSTES PROJECT - SESSION CONTEXT LOG
...
## BUILD #103 — SESSION LOG (2026-02-12, continuation)

### What Was Done This Session

#### 1. Integration Test Suite Fix (23 failures → 0)
- Flask-Limiter: `limiter.enabled = False` in fixture (config flag alone doesn't work)
- Custom rate limiter: clear `_deal_link_rate` dict between tests
- Airport API: unwrap from `{"airports": [...]}` dict wrapper
- Wallet addresses: changed to >= 25 chars (server validates)
- Helper tests: added wallet + card prerequisites before activate
- Browser session: assert 410 (feature removed Build #89)
- ProxyManager: added missing class to server.py for admin routes

#### 2. Stripe Payment Flow Fixes (ALL 3 critical issues resolved)
- **Webhook user attribution**: user_id stored in Stripe metadata, webhook resolves via metadata → email → deal owner fallback
- **Duplicate payment prevention**: check by stripe_payment_intent + stripe_session_id before creating records
- **Webhook error handling**: return 200 for processing errors (prevent retries), 400 only for signature failures
- Added refund flow: `create_stripe_refund()` in payments.py
- Added metadata validation: payment success handler validates deal_id from Stripe matches URL
- Added expired session handling in webhook
- 8 new Stripe tests added to test_api_clients.py

#### 3. P2P Test Suite Fix
- browser_control module removed in Build #89 but test_p2p.py still imported it at top level
- Made import conditional with try/except, skip TestBrowserControl class + 2 enum tests
- Fixed orchestrator test patches: `p2p_orchestrator.X` → `models.X` (all imports are lazy)
- Added `patch.dict("sys.modules", {"browser_control": MagicMock()})` for fail_transaction tests
- Result: 27 passed, 31 skipped (browser_control tests), 0 failed

#### 4. CI Pipeline Fix
- Added test_api_clients.py and test_p2p.py to CI (were missing — only ran test_integration.py)
- test_extension_pipeline.py has pre-existing fixture failures (not addressed)

#### 5. liteAPI Production Key Research
- Self-serve via dashboard, no IATA needed
- Request prod key with LLC/EIN → 1-3 business day turnaround
- Prod key prefix: `prod_` (vs `sand_`), zero code changes
- No per-API-call fees, commission baked into net rate

### Test Results (Build #103 final)
| Suite | Tests | Status |
|-------|-------|--------|
| `tests/test_integration.py` | 52 | ALL PASSING |
| `tests/test_api_clients.py` | 47 | ALL PASSING (39 + 8 Stripe) |
| `tests/test_p2p.py` | 27 passed, 31 skipped | ALL PASSING |
| `tests/test_extension_pipeline.py` | N/A | Pre-existing fixture failures |
| **Total** | **126 passed, 31 skipped** | **ALL PASSING** |

---

## BUILD #102 — SESSION LOG (2026-02-12)

### What Was Done This Session

#### 1. liteAPI Hotel Client (COMPLETE)
- Created `liteapi_client.py` as drop-in replacement for dead `amadeus_hotel_client.py`
- Full client: search (POST /hotels/rates), prebook (POST /rates/prebook), book (POST /rates/book)
- Key fix: iataCode param unreliable — resolved to cityName+countryCode via airports.py
- Key fix: hotel names not in rates response — fetched from GET /data/hotel endpoint
- Updated all consumers: mystes_ai.py, server.py (2 locations), trip_bundle.py, vertical_pipelines.py, data_source_resolver.py
- config.py + .env updated with LITEAPI_KEY
- Tested: Paris (Mercure Montmartre $136/night), New York (Sheraton, The Plaza)

#### 2. API Provider Signups (IN PROGRESS)
- **Duffel**: Application submitted with EIN doc (CP 575G), waiting approval
- **Picasso Travel / AERTiCKET**: Emailed customer relations + Kim, waiting response
  - CONFIRMED: 102-country POS multi-market consolidator, 55M fares, Cockpit API, 105 markets
- **liteAPI**: ACTIVE — sandbox key working
...
- User collected this data in sessions prior to Build #101
- Nearly every domestic route showed cheaper pricing through European POS
- This finding validates the entire MYSTES business model
- Picasso Travel's 102-country POS consolidator will replace proxy-based discovery with API access

### User Intent Log (Build #102)
...
## BUILD #101 — SESSION LOG (2026-02-12)

### What Was Done This Session

#### 1. PHOENIX → MYSTES REBRAND (COMPLETE)
- **Scope**: 1,572 occurrences across 125 files
- **Files renamed**: phoenix_ai.py → mystes_ai.py, phoenix_agent.py → mystes_agent.py, phoenix_ai_api.py → mystes_ai_api.py, phoenix_intelligence.py → mystes_intelligence.py, generate_phoenix_report.py → generate_mystes_report.py, phoenix_extension/ → mystes_extension/, PHOENIX_PORTFOLIO.md → MYSTES_PORTFOLIO.md, PHOENIX_PORTFOLIO.html → MYSTES_PORTFOLIO.html, PHOENIX_MODEL_2.md → MYSTES_MODEL_2.md
- **Android package**: com.phoenix.app → com.mystes.app (directory + content)
- **iOS/Xcode**: All references updated in project.pbxproj, Info.plist, AppDelegate.swift
- **Content replaced**: All imports, class names (PhoenixAI → MystesAI, PhoenixAgent → MystesAgent), CSS variables (--phoenix-glow → --mystes-glow), env vars (PHOENIX_ECONOMIC_PHASE → MYSTES_ECONOMIC_PHASE), DB name (phoenix.db → mystes.db)
- **Preserved**: Migration files (historical DB records), package-lock.json (auto-generated)
- **UI rule**: MYSTES must ALWAYS appear in ALL CAPS in the UI. Never "Mystes" — always "MYSTES"

#### 2. MYSTES BRAND TYPOGRAPHY (COMPLETE)
- **Font**: Cinzel (Google Fonts) — Pythagorean/classical initiate aesthetic
- **CSS variable**: `--font-brand: 'Cinzel', 'Trajan Pro', 'Palatino Linotype', serif`
- **Applied to**: Nav wordmark, loading screen text, footer brand, landing hero, about page, AI chat welcome heading
- **Gold gradient**: `linear-gradient(135deg, #ffffff 0%, #e8d5b7 40%, #ffffff 60%, #c9a96e 100%)` with background-clip text
- **Utility class**: `.mystes-brand` — applies font, uppercase, letter-spacing, gold gradient
- **All MYSTES text** uses wide letter-spacing (5-12px) for that secret order feel

#### 3. HOTEL FRONTEND BUILT → THEN CONSOLIDATED INTO AI (COMPLETE)
- **Initially built**: Separate /hotels page with search form, HOTELS_SEARCH_CONTENT template, /api/hotels/search and /api/hotels/select endpoints, HOTEL_BOOK_CONTENT checkout template, hotel booking fulfillment, database migration for hotel columns on Deal + Booking models
- **Then consolidated per architecture decision**: Removed Hotels from primary nav. All hotel search now goes through MYSTES AI conversational interface.
- **What changed in consolidation**:
  - `mystes_ai.py`: search_hotels tool now calls `amadeus_hotel_client.search_hotels()` directly (was routing to arbitrage_engine.search())
  - Tool schema updated: location, checkin_date, checkout_date, adults, rooms
  - City name → IATA code resolution via airports.py AIRPORTS dict
  - New `_format_hotels()` formatter returns structured hotel data for the LLM
  - System prompt updated with hotel search instructions, defaults, presentation guidelines
  - `server.py`: New `renderHotelCards()` JS renderer in MYSTES_AI_CONTENT — shows hotel name, room type, price/night, total, nights, cancellation, Book button
  - New `bookHotel()` JS function calls `/api/hotels/select` → creates Deal → redirects to /book/<deal_id>
  - Tool dispatcher updated: search_hotels → renderHotelCards()

#### 4. LLC + BUSINESS FORMATION + API PROVIDER PIVOT
- **Tennessee LLC**: Filed via TN Secretary of State (Form SS-4270), $300 fee, member-managed, NAICS 561510
- **EIN**: Assigned by IRS, no Form 720 needed
- **Sky Bird Travel**: REJECTED — they do not work with online OTAs
- **Amadeus Self-Service**: DISCONTINUED — portal shut down, Enterprise only now
- **PIVOT (2026-02-12)**: Switching from Amadeus to modern APIs that accept OTAs:
  - **Duffel API (flights)**: Self-serve, no IATA needed, 300+ airlines, $3+1%/booking. Missing Delta + Southwest.
  - **Picasso Travel / AERTicket (consolidator)**: Explicitly serves OTAs, Cockpit REST API, GDS+NDC 100+ markets.
  - **liteAPI / Nuitee (hotels)**: 2M+ hotels, self-serve, free core API, developer-first.
  - **Hotelbeds (hotels backup)**: Free sandbox, 180K+ hotels, production needs sales approval.
- **Codebase impact**: Replace amadeus_client.py + amadeus_hotel_client.py with new provider clients. Everything else stays.
- **Dead paths**: Mondee/WINGS (Chapter 7 bankruptcy), Booking.com API (closed), Amadeus Self-Service (discontinued)

### Current Project Stats (2026-02-12)
| File | Lines | Notes |
|------|-------|-------|
| server.py | 21,141 | 248 routes, all templates as Python strings |
| models.py | 4,189 | 75 database models |
| mystes_ai.py | 3,069 | 30+ tools, Anthropic Messages API |
| base_template.py | 2,247 | Cinzel + Outfit fonts, aurora background |
| Total Python files | 79 | Root directory |

### Key Architecture Reminders
- **ALL verticals go through MYSTES AI chat** — no separate search pages per vertical (Decision #8)
...
- Stripe (production-ready — webhook, refunds, duplicate prevention all wired up; needs live keys)
...
- Manual agent
- **P2P Network (System 2)** — helper in target market executes purchase via Mystes-controlled browser session
...
- **How it works:**
  ...
  2. Helper in target market grants Mystes temporary browser control via client app
  3. Mystes automates the purchase on helper's device (same Playwright scripts, different execution env)
...
- **No Mystes seed capital needed:** Buyer's RLUSD is locked before helper purchases. Helper fronts with credit card, gets RLUSD immediately on confirmation.
- **Token system (RESERVE — only if needed):** Mystes token on XRPL backed 1:1 by RLUSD. Only build if direct RLUSD payments create friction. Not planned for initial build.
...
- **Exchange on-ramp:** Users need to acquire RLUSD/XRP to fund their wallet. Primary: Coinbase. Global alternatives: Binance, Kraken, Uphold, Bitstamp, Crypto.com. Embedded buy widget (MoonPay or Transak) for in-app purchases without leaving Mystes. Show exchange recommendations by user's country/region.

### 7. Commercial Account Onboarding — DECIDED 2026-01-29
- **Purpose:** Onboard travel agencies, OTAs, and corporate travel desks to use Mystes as their arbitrage engine via API.
- **Fee model:** Percentage of realized savings ONLY. No minimum fee. No charge if no savings found. Pure value alignment — Mystes earns only when it delivers value.
...
- **Use-it-or-lose-it mechanic:** If volume drops below tier threshold for 2 consecutive 30-day periods, rate cut suspends (tier downgrades). Incentivizes agencies to keep pushing volume through Mystes.
...
- **Agency flywheel:** Agencies onboard their clients to Mystes portal → clients link Google accounts → Mystes handles booking → agencies get savings kickbacks (lower fee tiers). Clients become passive P2P helper nodes, expanding Mystes's residential proxy network. More clients = more markets = better scraping = more savings for everyone.
...
- **"Citizen API" concept:** People-powered market data layer accessible to all participants. Mystes becomes a provably honest API built on real user data, not institutional data feeds. GDS/ATPCO don't provide real consumer-facing prices — Mystes does.
...
- **Helper payout model:**
  ...
  - Equilibrium: Mystes $65-95M/month revenue, avg active helper $100-200/month, passive node $12-20/month (at 5% capture)
- **CitizenSERP Micropayment Model (Node Payout from Airline SaaS Revenue):**
  - Residential proxy nodes (Mystes users) are the data collection layer powering the Airline Intelligence product
...
- **Multi-Vertical Expansion — Universal Arbitrage Search Engine:**
  - Mystes is NOT a flight app — flights are vertical #1. The platform expands to all goods and services with geographic pricing arbitrage.
  - CitizenSERP is the infrastructure layer INSIDE Mystes — the decentralized residential proxy network that powers all verticals. It is not a separate product.
  - **Expansion path:** Flights (built) → Hotels → Cruises → E-Commerce (fashion, electronics) → Software/Digital → Universal search
  - **Endgame:** User types any product or service into Mystes search, CitizenSERP nodes fan out across markets, return price matrix with arbitrage opportunities. "Carhartt jacket" returns US vs Japan vs UK prices. "JFK to LAX March 1" returns flight prices by market. Same search bar, same node network.
...
- **Cross-Market Direct Access — Disintermediation Engine:**
  - Mystes eliminates middlemen/gatekeepers (importers, brokers, resellers) by giving users direct proxy access to foreign marketplaces
  - Example: US user wants a JDM import car. Today they can only find US importers charging 2x markup. Mystes provides proxy gateway so they browse Facebook Marketplace Japan directly, see listings locals see, at local prices
  - **Proxy gateway model:** User stays logged into their OWN social/marketplace account (Facebook, etc). Mystes piggybacks proxy location so the platform's geo-restrictions are bypassed. User sees foreign listings through their own authenticated session.
  - **Communication:** No custom messaging system needed. User is logged into their own Facebook → they use Facebook Messenger directly to contact the seller. Same applies to WhatsApp, Telegram, or any native messaging on the platform. Mystes just provides the geographic access — the existing communication infrastructure handles the rest.
  ...
  - **This is the endgame vision:** Mystes doesn't just find price differences — it connects people directly across markets that were previously invisible to each other. The proxy network becomes a market access layer, not just a price comparison tool.
...
- **Mystes AI Search Engine:**
  ...
  - BYOAI (primary path): users log into their own AI subscription (ChatGPT, Claude, Gemini, etc.) through the proxy portal with their own credentials (Google SSO, etc.). Their AI sees data from the proxy's geographic location. No API key needed — user's existing subscription handles billing, Mystes provides the geographic tunnel. The AI category in the proxy portal app directory (21 LLM sites) IS the BYOAI onboarding mechanism.
  - BYOAI (advanced path): users can also add their provider API key to Mystes's ensemble search engine (no Mystes credit cost when using own keys)
...
- **Everything runs through Mystes — data at every touchpoint:**
  - Mystes is the data layer even when "just" providing proxy access. Every interaction generates capturable market intelligence.
  ...
  - **Core principle: Mystes learns from every transaction, every query, every session. This accumulated intelligence IS the product that enterprises pay for.**
- **DO NOT:** Frame Mystes as adversarial to airlines. DO NOT build airline product as a booking tool (data-only SaaS). DO NOT promise savings percentages won't compress at scale. DO NOT build CitizenSERP payouts without on-chain uptime verification. DO NOT build new verticals as separate products — they are vertical plugins within Mystes. DO NOT build custom messaging — leverage existing platform communication (Facebook Messenger, WhatsApp, Telegram). DO NOT build a marketplace — Mystes provides access to existing marketplaces across geographic boundaries.

### 9. MystesAI Agent Architecture — DECIDED 2026-01-30
- **MystesAI is the brain. CitizenSERP is the engine. RLUSD is the fuel.**
...
- **Three coexisting AI systems (NOT redundant):**
  1. **Existing search pipeline (tool):** User-driven — user types query, AI ensemble answers with Mystes data context. Already built.
  2. **BYOAI via proxy (infrastructure):** User's own AI subscription through geographic tunnel. Already built.
  3. **Agent mode (intelligence):** MystesAI itself makes decisions — which markets to search, which nodes to dispatch, how to interpret results. THIS is what's being built.
- **"Learning" = data accumulation, NOT LLM training:** LLMs don't learn between sessions. Mystes's database IS the memory. More searches = richer data = better AI context. Three feedback loops: Route Learning (which market was chosen), Query Pattern Learning (what users ask about), Outcome Learning (did the booking succeed, what were actual savings).
- **Intelligence routes ARE agent tools:** Each `/api/intelligence/*` route simultaneously serves as user-facing dashboard, commercial API data product, and callable tool for the MystesAI agent. Triple-use by design.
...
- **CitizenSERP task types:** Expanding from generic search to typed tasks: `flight_search`, `product_search`, `hotel_search`, `marketplace_browse`, `general_search`. Each task type tells the node what to search and how to extract structured data. MystesAI decides which task type to dispatch and to which markets.
- **Self-upgrading:** MystesAI reads its own telemetry (query patterns, proxy usage by site, alert demand) and discovers new arbitrage opportunities autonomously. It doesn't need anyone to manually add "hotel search" — it sees query patterns and creates new search types.
- **The flywheel:** More searches → more node payments → more nodes join → broader geographic coverage → better data → MystesAI finds new patterns → creates new search types → more searches.
...
- **DO NOT:** Build agent as separate from CitizenSERP. DO NOT build verticals with separate infrastructure. DO NOT train/fine-tune LLMs — accumulate data in Mystes's DB instead. DO NOT frame MystesAI as redundant to BYOAI — they serve different purposes.
...
| templates/base_template.py | Mystes UI template (aurora/cosmic theme) — nav updated with Earn/Helper/Wallet links |
...
| mystes_agent.py (~1165 lines) | **UPDATED** MystesAI agent orchestrator: AgentTools (12 intelligence tools), MarketSelector (zone-level selection), MystesAgent (search, discover, analyze), intent parsing (10 verticals), universal_search (smart hybrid dispatcher), search_flights/search_cruise/search_ecommerce/search_digital, cruise+digital demand discovery signals |
...
| serp_api.py (~905 lines) | **NEW** Mystes Residential SERP API Service: SERPAPIManager class (validate, credits, quota, rate limit, sync/async execution, task dispatch, result formatting, usage tracking, monitoring), SERP_TIERS (6 tiers: free→partner), CREDIT_MULTIPLIERS (8 options), SUPPORTED_ENGINES (8 engines: Google/Bing/DuckDuckGo/Yahoo/Yandex/Baidu/Naver/Yahoo Japan), GOOGLE_DOMAINS (15 country domains) |
...
| ai_search.py (~870 lines) | **UPDATED** MystesAI multi-provider ensemble search engine + private market escrow builder + P2P deal links with single-link revocation + share_links integration. 9 providers, parallel query, BYOAI, deal contract builder, XRPL escrow, link gen/accept/email |
...
| node_service.py (~500 lines) | **NEW** Local node service daemon (aiohttp): MystesNodeService class on localhost:19750. HTTP handlers: /status, /events, /config, /proxy.pac, /stop. Background loops: heartbeat (60s), batch upload (30s), config refresh (5min). Reconnection with exponential backoff (1s→60s). CLI with --token/--server/--port/--proxy-mode/--verbose |
| mystes_extension/ | **NEW** Chrome Manifest V3 extension: manifest.json (activeTab, storage, alarms, webNavigation permissions), background.js (~250 lines, service worker: status pings, event buffering, flush to localhost:19750), content.js (~350 lines, DOM extractors: search queries, ads, prices, social signals, MutationObserver for SPA), popup.html/js (earnings + status display), options.html/js (server URL, token, port config) |
...
- Built UI templates (Mystes aurora theme)
...
14. **User clarified P2P escrow model** — Two options discussed: (A) lock buyer funds, release to helper pre-purchase (has counterparty risk), (B) Mystes provides seed liquidity while buyer funds locked (eliminates risk but requires capital). User initially favored Model B.
15. **User clarified Mystes controls the browser** — Helper doesn't manually navigate. Mystes takes remote control of helper's browser session via client app. Same Playwright automation as System 1, just executing on helper's device instead of through a proxy. Helper is a trusted endpoint, not an operator.
16. **User proposed token system as reserve option** — Mystes token backed 1:1 by RLUSD stablecoins. Only deploy if needed. Not for initial build.
17. **User solved the seed capital problem** — Buyer converts USD → RLUSD upfront, locks in escrow. Helper sees on-chain escrow, trusts it, fronts purchase with their credit card. Escrow releases RLUSD to helper on confirmation. No Mystes treasury needed. RLUSD circulates in ecosystem — helpers spend earned RLUSD on their own flights. Closed-loop economy.
18. **User confirmed on-chain escrow verification** — Both buyer and helper can inspect locked escrow directly on XRPL ledger. This is the trust foundation — neither party relies on Mystes's word, they verify on-chain.
...
20. **User requested crypto exchange on-ramp** — Users need access to exchanges to acquire crypto/RLUSD for their Mystes wallet. Support Coinbase (primary) plus major global exchanges (Binance, Kraken, Uphold, Bitstamp, Crypto.com). Embedded buy widgets (MoonPay/Transak) recommended for seamless in-app purchases without leaving Mystes.
21. **User requested /earn page for P2P helper recruitment** — Build a pitch page advertising the helper income system. Key messaging: earn XRP passively by granting Google account access, Mystes rents access to established accounts instead of paying data farms, helpers seed the network and earn from every purchase through their region, "take back your data" angle — users benefit from their data instead of institutions. Built /earn route + EARN_CONTENT in server.py, added "Earn" link to nav bar.
22. **User articulated broader Mystes mission** — Mystes as a "citizen SerpAPI" — an honest, people-powered data network. Mission: expose that Google/airlines gatekeep geopricing data to exploit consumers. Build a decentralized residential network of real users that corporations cannot stop. Passive earning model: users just allow Mystes access to run in background, Mystes handles all automation. Vision: Mystes becomes a provably honest API that demonstrates big tech/travel corps manipulate data for market advantage. Supply-side moat: network of opted-in real users is not replicable like proxy farms. Key distinction noted: Google doesn't fabricate prices — the manipulation is in the opacity (no disclosure of cross-border price differences).
...
24. **User requested building all roadmap items** — Built everything that doesn't require purchasing external services:
    ...
    - **email_service.py (updated +190 lines)**: Added 6 P2P email templates — send_p2p_escrow_locked(), send_p2p_helper_matched(), send_p2p_booking_confirmed(), send_p2p_helper_payment(), send_p2p_transaction_failed(), send_p2p_escrow_refunded(). All use Mystes branded HTML layout.
...
25. **User requested building all 6 roadmap items ("lets dive into all and build")** — Built everything:
    ...
    - **Deployment config**:
      - Dockerfile: Multi-stage build (builder + production), Playwright system deps, non-root mystes user, gunicorn.conf.py CMD
      - docker-compose.yml: 4 services (web, postgres, redis, websocket), health checks, named volumes, mystes-network
...
32. **Captured core mission and market position as Architectural Decision #8**:
    - Mystes is not choosing a side — it is choosing everyone
    ...
    - MAD strategic position: Mystes earns whether airlines subscribe or don't
...
36. **Built 4 architectural hardening items** (2 of 6 identified were already implemented):
    ...
    - **server.py: Security hardening**: Security headers via @after_request (X-Frame-Options DENY, X-Content-Type-Options nosniff, X-XSS-Protection, Referrer-Policy, Permissions-Policy, HSTS on HTTPS). Rate limit on /api/p2p/match (30/hr) + login_required. Audit logger (mystes.audit) with audit_log() helper for financial events (p2p_book, payment_verified, data_export, account_deletion)
...
38. **User declared multi-vertical expansion vision** — Mystes is a universal arbitrage search engine, not a flight app. Flights are vertical #1. Expansion: hotels → cruises → e-commerce (fashion, electronics) → software/digital → universal search. User wants anyone to type any product/service and see arbitrage opportunities across markets. CitizenSERP is the infrastructure layer inside Mystes, not a separate product. Existing flight code stays as-is — new verticals are additive. At 3-4 verticals, passive node earnings reach ~$25-30/month at 1M nodes due to multiple data subscriber pools sharing the same node network.
39. **User articulated cross-market disintermediation vision** — Mystes eliminates middlemen (importers, brokers) by providing proxy gateways to foreign marketplaces. Example: JDM import car from Japan — US buyer currently can only find importers at 2x markup. Mystes lets them browse Facebook Marketplace Japan directly and find the seller at half the price. Communication via existing platform messaging (Facebook Messenger, Telegram, WhatsApp) — no custom messaging system needed. Translation layer is a future enhancement. Hard goods have shipping/customs friction but the transparency alone births new markets by connecting people directly.
40. **User clarified proxy gateway model** — User stays logged into their OWN Facebook account. Mystes piggybacks the proxy location so Facebook's geo-restrictions on Marketplace are bypassed. User sees foreign listings through their own authenticated session. They contact sellers directly via Messenger. No intermediary platform needed — Mystes just provides geographic access. This is the simplest, most powerful model: leverage existing marketplace + messaging infrastructure, just remove the geographic wall.
41. **User requested universal proxy portal ("is there a universal way to allow any app or site to plug in")** — Wants on-ramps to social media, messaging apps, marketplaces all accessible through Mystes's proxy infrastructure. The universal model is the proxy itself — no per-app integration needed. Built a Proxy Portal with curated app directory + proxy session management.
...
49. **Built Mystes Intelligence Engine — data-aware AI**:
    ...
    - **Data flow**: User query → intelligence parses for route → pulls SearchHistory/PriceHistory/Deal data → formats as context → injects into LLM system prompt → AI answers with REAL Mystes data instead of guesses

50. **Intelligence Engine Phase 2 — Full Intelligence API (10 new endpoints)**:
    - **mystes_intelligence.py (+350 lines, 10 new methods)**: `get_p2p_network(days_back)` — helper coverage by country, transaction funnel (status counts), dispute rate + resolution breakdown. `get_p2p_savings(days_back, limit)` — top savings routes grouped by origin/dest/market. `get_node_network()` — delegates to citizenserp_manager.get_network_stats(). `get_proxy_usage(days_back)` — portal sessions, top target sites, sessions by country. `get_ai_analytics(days_back)` — extends provider performance with daily query volume, BYOAI adoption counts, top queried markets, total credits. `get_alert_demand(limit)` — most-watched routes from active PriceAlerts, avg price/savings thresholds. `get_private_market_stats(days_back)` — deal volume by status/type, avg deal size, risk score, completion rate, escrow fees. `get_payment_analytics(days_back)` — payment method distribution, daily revenue trend, conversion rate. `get_price_timeline(origin, dest, days_back)` — delegates to tracker.get_price_trend(). `get_airline_comparison(origin, dest, days_back)` — airline pricing from CompetitorPricing + ancillary fees from AncillarySnapshot, ranks by total cost.
    ...
    - **Purpose**: Triple-use — user-facing intelligence dashboards, commercial API data products, and future agent tool kit (each route becomes a callable tool for MystesAI agent)
...
52. **MystesAI Agent Orchestrator — autonomous intelligence layer**:
    ...
    - All 4 files pass `python3 -m py_compile`
    - **Architecture**: Agent uses intelligence routes as tools → dispatches typed tasks to CitizenSERP nodes → aggregates results → AI ensemble analysis. Every search pays the node. Every search feeds Mystes's data layer. Self-upgrading via opportunity discovery.
...
53. **Geographic Zone System — Sub-Regional Precision Arbitrage**:
    ...
    - **mystes_agent.py (MODIFIED +60 lines)**: `MarketSelector.select_zones()` — adds intra-country zones for home market, expands foreign countries to major zones, boosts zones with active nodes, deduplicates by confidence. `discover_opportunities()` gained `intra_country_arbitrage` opportunity type.
...
61. **Private P2P Deal Links — Shareable Escrow for Foreign Marketplace Transactions**:
    - **Problem**: Users browsing foreign marketplaces via Mystes proxy (e.g., Facebook Marketplace Japan) had no transaction vehicle to settle with sellers. The market comparison model (flights, hotels, products) doesn't apply to one-off private listings.
    - **Solution**: Shareable deal links that create XRPL escrow between a Mystes buyer and an external seller who does NOT need a Mystes account.
    ...
    - **ai_search.py (MODIFIED +200 lines)**: Added `_validate_xrpl_address()` module-level helper (regex + optional xrpl-py). Updated `create_private_deal()` — seller_wallet now optional, accepts seller_email/seller_name. Updated `fund_deal_escrow()` — accepts both 'draft' and 'seller_accepted' status, validates seller_wallet is set. 6 new MystesAI methods: `generate_deal_link(deal_id, user_id, expiration_days)` — generates secrets.token_urlsafe(32), sets status=link_sent, emits SSE; `send_deal_link_email(deal_id, user_id)` — sends HTML email via email_service with deal terms + escrow explainer + CTA button; `get_deal_by_link_token(link_token)` — public lookup, sanitised response, checks expiration; `accept_deal_link(link_token, seller_wallet, seller_name)` — XRPL address validation, sets seller_wallet + status=seller_accepted, emits SSE to buyer; `mark_deal_link_viewed(link_token)` — analytics, first-view timestamp + SSE to buyer; `cleanup_expired_deal_links()` — batch marks expired links.
    - **server.py (MODIFIED +120 lines, 5 new routes)**: Updated `POST /api/portal/deal/create` — seller_wallet no longer required. 3 new authenticated routes: `POST /api/portal/deal/<id>/generate-link` (1-30 day expiration), `POST /api/portal/deal/<id>/send-link` (email to seller), `GET /api/portal/deal/<id>/link-status` (viewed/accepted/expired). 2 new PUBLIC routes (no auth): `GET /deal/link/<token>` (self-contained HTML page with MYSTES branding, deal terms, price breakdown, escrow explainer, wallet input form, JS fetch-based acceptance), `POST /deal/link/<token>/accept` (rate-limited 3/min per IP, XRPL wallet validation). In-memory rate limiter `_check_rate_limit()`.
    - **migration k1l2m3n4o5p6**: ALTER seller_wallet nullable, ADD 6 columns, CREATE UNIQUE INDEX on link_token.
    - All 4 files pass `python3 -m py_compile`.
    - **Key design**: Seller needs zero Mystes infrastructure — just a browser and an XRPL wallet. Link token is the auth. Backwards compatible — existing direct-wallet deal flow unchanged. Email failure gracefully degraded (buyer can share URL manually).

62. **Multi-Platform Share Links + Smart Contract Templates + Seller Onboarding**:
    - **Problem**: Deal links and referral links existed but had no platform-specific sharing — users had to manually copy/paste URLs. No contract templates for common transaction types. Sellers accepting deals had no onboarding path to become Mystes users.
    ...
    - **models.py (MODIFIED +1 line)**: `User.onboarded_from_deal_id` — Integer, nullable. Tracks which deal brought a seller into Mystes as a user.
...
72. **Build #72 — Mystes AI: Conversational Interface + Tool Orchestration**:
    - **Problem**: Users interact with Mystes through manual form-based UI. Need a modern AI-powered conversational interface as a premium layer that can access all Mystes capabilities via natural language.
    - **Solution**: Mystes AI — LLM-powered chat interface with tool-calling orchestration. Dual-function: standard Mystes features remain accessible without AI; Mystes AI is a tiered subscription.
    - **mystes_ai.py (NEW ~1400 lines)**: MystesAI class — tool-calling orchestrator using Anthropic Messages API (claude-sonnet-4-20250514). 14 tool definitions mapping to existing modules: search_flights→search_global, search_hotels/products→mystes_agent, analyze_route→mystes_agent.analyze_route, get_route_intelligence/market_briefing/trending/price_history→intelligence module, browse_proxy→proxy session, serp_search→serp_api_manager, get_node_status→node_registry, get_earnings→yield_dashboard, get_deals→Deal.query, discover_opportunities→mystes_agent. MYSTES_AI_TIERS dict (4 tiers: ai_free/0$/25q, ai_starter/$19/500q, ai_professional/$49/2000q, ai_unlimited/$99/unlimited). Node operators get 25% discount. Rich content formatting (HTML tables, charts, cards). Multi-turn context management.
    - **mystes_ai_api.py (NEW ~350 lines)**: register_mystes_ai_routes(app). 7 routes: POST /api/v1/ai/chat (SSE streaming chat with tier quota + rate limit enforcement), GET /api/v1/ai/conversations (list user conversations), POST /api/v1/ai/conversations (create new), GET /api/v1/ai/conversations/<id> (history), DELETE /api/v1/ai/conversations/<id>, GET /api/v1/ai/tier (user tier + usage), GET /api/v1/ai/pricing (public tier options). In-memory sliding window rate limiting per user. Per-query credit system (1 query = 1 credit, tool calls free).
...
68. **Post-Build #67 — Token Flow, Tests, Icons, Background Worker, Dashboard**:
    ...
    - **mystes_extension/icons/ (NEW, 3 PNG files)**: icon16.png, icon48.png, icon128.png — orange "P" on dark circle, generated programmatically as valid PNGs.
...
66. **Vertical Completion — Gaps Fixed (Build #66)**:
    ...
    - **mystes_agent.py (MODIFIED +14 lines)**: Added `search_flights(origin, destination, date)` method — flights now have a dedicated agent method like all other verticals.
...
65. **Enhanced Data Extraction + Node Yield Dashboard + Ad Intelligence**:
    - **Problem**: Mystes nodes earn flat uptime payouts regardless of data value. No visibility into earnings by data category. No ad intelligence pipeline despite ad data being the highest-value category.
...
63. **Four New Verticals + Transaction Fee Engine + Search Engines + Universal Search**:
    - **Problem**: Mystes only had 4 verticals (flights, hotels, products, marketplace). No fee model to monetize arbitrage savings. No cross-vertical search. No search engine onboarding portal.
    ...
    - All 4 files pass `python3 -m py_compile`.
    - **Key design decisions**: (1) Two fee models — arbitrage takes % of savings Mystes finds (25% default, reduced by tier), private market takes flat $1.50 + 1% (no arbitrage possible). (2) ALL transaction types (arbitrage + private market) count toward rewards tier progression. (3) Minimum floor on arbitrage fee = private market fee ($1.50 + 1%). (4) Universal Search is NOT a CitizenSERP task — pure orchestration dispatching existing task types concurrently. (5) CruisePipeline uses per-person-per-night for fair comparison across durations. (6) ECommercePipeline coexists with ProductPipeline — broader scope + category detection. (7) DigitalPipeline has NO landed cost — pure currency conversion + region-lock tracking. (8) New verticals checked FIRST in intent detection to avoid keyword overlap with existing product_search.
    - **Future build specs (documented, not coded)**: Browser extension (auto-detect shopping pages, show arbitrage badge), Telegram bot (@MystesArbitrageBot with /search, /compare, /alerts, /deals, /wallet), Crypto wallet integration (XRPL wallet discovery, Xaman deep link, browser wallet detection).
    - **Future concept: Device-Wide Data Monetization (idea #3 from Prompt #65)**: User installs Mystes app/VPN that intercepts app telemetry and ad tracking from all device apps (Facebook, Google, TikTok etc). Mystes becomes the intermediary — blocks unwanted data collection, sells consented anonymized/aggregated behavioral data through Mystes data marketplace, returns revenue to node operators. Requires VPN/DNS filter layer, app store compliance (ATT on iOS, Privacy Sandbox on Android), data marketplace integration. Separate product phase — build after core proxy + SERP API revenue is flowing. Competitive landscape: Brave (BAT), Permission.io, Swash. Mystes advantage: XRPL settlement rail already built.
