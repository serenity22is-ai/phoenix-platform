# MYSTES PLATFORM — STRUCTURAL REFERENCE v2
...
**Mystes is NOT a flight app.** Flights are the entry point. The platform becomes infrastructure for all arbitrage discovery.
...
- Mystes books via Amadeus, user saves money, Mystes takes a cut

**Example:**
- Amadeus prices flight at $600
- Google shows consumers $750
- User saves $150
- Mystes takes 25% ($37.50)
- User keeps $112.50
...
- Mystes takes 25% of savings (or less based on tier)
- User keeps the rest

**Tier System:**
- Users get better tiers by running nodes
- Better tier = Mystes takes smaller cut = user keeps more savings
...
- Mystes finds cheapest regional price
- Mystes ALWAYS handles the booking — user never interacts with nodes
...
- When competitors try to race to bottom on pricing, Mystes wins
- Traditional proxy providers have fixed cost floor (infrastructure, IP leases, bandwidth)
- Mystes cost basis is near-zero (infrastructure = users' devices)
- Mystes can pay nodes MORE while charging customers LESS than competitors can break even
...
**Endgame:**
- Mystes becomes infrastructure — immovable part of data industry
...
Tiers determine Mystes's cut of arbitrage savings. Better tier = user keeps more.

| Tier | Mystes Takes | User Keeps | How to Achieve |
|------|---------------|------------|----------------|
...
### Phase 1 (Now)
- Running a node = better tier = smaller Mystes cut on your bookings
- No direct payments
...
**Payment Flow:**
1. Mystes sells proxy bandwidth per GB to external customers
...
- Mystes: Infrastructure is users' devices. Cost basis near-zero.
- Mystes can ALWAYS undercut competitors while paying nodes more
- Race to bottom = Mystes wins, competitors bleed
...
- Agencies/OTAs access Mystes arbitrage via API
...
- Airlines that don't subscribe get exposed — Mystes offers their passengers better pricing through competitors
- Mystes earns either way
...
**How it works:**
1. Helper in foreign market grants Mystes browser control via client app (helper_client.py)
2. Mystes automates purchase on helper's device via Playwright (browser_control.py)
...
**Key Points:**
- No Mystes seed capital needed — buyer's RLUSD locked before helper purchases
...
- Primary model: Mystes books via Amadeus, no helper involvement
...
The decentralized residential proxy network. Infrastructure layer INSIDE Mystes, not a separate product.
...
| mystes_agent.py | AI orchestration layer |
| mystes_intelligence.py | Data-aware AI context building |
...
**Database:** mystes_db_5717, user: mystes
...
- Frame Mystes as adversarial to airlines
...
3. **Tier System Implementation**
   ...
   - Define exact Mystes cut per tier
