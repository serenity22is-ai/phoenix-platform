/**
 * PHOENIX WebView Event Capture (Build #91)
 *
 * Simplified port of phoenix_extension/content.js extraction logic
 * for the Capacitor WebView context. Instead of chrome.runtime.sendMessage(),
 * events are routed to PhoenixNodeClient.captureEvent().
 *
 * Requires: phoenix-node-client.js loaded first (window.PhoenixNodeClient)
 *
 * Usage:
 *   const capture = new PhoenixNodeCapture(nodeClient);
...
(function (root) {
  ...
  class PhoenixNodeCapture {
    /**
     * @param {PhoenixNodeClient} nodeClient  The node client to send events to
    ...
    start() {
      ...
      console.log("[PhoenixCapture] Started WebView event capture");
    }
    ...
    stop() {
      ...
      console.log("[PhoenixCapture] Stopped WebView event capture");
    }
  ...
  root.PhoenixNodeCapture = PhoenixNodeCapture;
