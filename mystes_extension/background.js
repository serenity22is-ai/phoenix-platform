// Mystes Extension — Background Service Worker (Build #91)
...
// Mystes backend using the user's helper_token. This enables the extension
...
const CONTENT_SCRIPT_ID = "mystes-content";

// Direct-backend fallback keys (Build #91)
const BACKEND_URL_KEY = "mystes_backend_url";
const HELPER_TOKEN_KEY = "mystes_helper_token";
...
async function registerContentScriptIfConsented() {
  ...
  try {
    ...
    if (anyEnabled) {
      ...
      console.log("[Mystes] Content script registered (consent active)");
    } else {
      console.log("[Mystes] Content script unregistered (no consent or onboarding incomplete)");
    }
  } catch (error) {
    console.warn("[Mystes] Content script registration error:", error.message);
  }
...
chrome.runtime.onInstalled.addListener((details) => {
  if (details.reason === "install") {
    ...
    console.log("[Mystes] First install — opening onboarding");
  }

  if (details.reason === "update") {
    console.log("[Mystes] Extension updated to", chrome.runtime.getManifest().version);
  }
...
async function checkServiceStatus() {
  try {
    ...
    if (localCheckFailures >= DIRECT_MODE_CONSECUTIVE_FAILURES && canUseDirectMode()) {
      if (!directMode) {
        directMode = true;
        console.log("[Mystes] Switching to direct-backend mode (local daemon unreachable)");
      }
    }

    await updateBadge();
    console.warn("[Mystes] Service unreachable:", error.message,
...
async function fetchConfig() {
  ...
  try {
    ...
    console.log("[Mystes] Config refreshed" + (directMode ? " (direct)" : ""));

    return data;
  } catch (error) {
    console.warn("[Mystes] Config fetch failed:", error.message);
...
async function flushBuffer() {
  ...
  try {
    ...
    console.log(`[Mystes] Flushed ${batchSize} events${directMode ? " (direct)" : ""}. Total sent: ${stats.sent}`);
    ...
    if (eventBuffer.length > MAX_BUFFER) {
      ...
      console.warn(`[Mystes] Buffer overflow, dropped ${dropped} oldest events`);
    }

    stats.errors++;
    console.warn("[Mystes] Flush failed:", error.message);
    return null;
  }
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.type === "set_backend_credentials") {
    ...
    if (backend_url && helper_token) {
      ...
      console.log("[Mystes] Backend credentials stored for direct mode");
      sendResponse({ ok: true });
    } else {
...
console.log("[Mystes] Background service worker starting...");
