#!/usr/bin/env python3
"""
PHOENIX Platform - Market Presentation Report Generator

Generates a comprehensive PDF presentation explaining the Phoenix flight
...
# Colors - Phoenix Fire Theme
PHOENIX_ORANGE = colors.HexColor("#FF6B35")
PHOENIX_GOLD = colors.HexColor("#FFC107")
...
def create_cover_page(styles):
    ...
    elements.append(Paragraph(
        "Phoenix brings these savings back to consumers through<br/>"
...
def create_executive_summary(styles):
    ...
    elements.append(Paragraph(
        "Phoenix has uncovered a systematic practice of geographic price discrimination "
    ...
    findings_table.setStyle(TableStyle([
        ('BACKGROUND', (0, 0), (-1, 0), PHOENIX_ORANGE),
    ...
    elements.append(Paragraph("The Phoenix Solution", styles['SubHeading']))
    elements.append(Paragraph(
        "Phoenix automatically searches flight prices across multiple global markets simultaneously "
        "using residential proxies. When a cheaper price is found in a foreign market, Phoenix "
...
def create_solution_section(styles):
    ...
    elements.append(Paragraph("THE SOLUTION: Phoenix Platform", styles['SectionHeading']))

    elements.append(Paragraph(
        "Phoenix is an automated flight price arbitrage platform that searches multiple global "
    ...
    elements.append(Paragraph("How Phoenix Works", styles['SubHeading']))

    steps = [
        "<b>Step 1: User Search</b> - User enters flight details (origin, destination, dates)",
        "<b>Step 2: Parallel Scraping</b> - Phoenix simultaneously opens browser sessions through proxies in US, Spain, UK, and other markets",
        ...
        "<b>Step 7: Smart Booking</b> - If user proceeds, Phoenix books through the cheapest market",
    ]

    for step in steps:
        elements.append(Paragraph(step, styles['PhoenixBody']))
    ...
    """
    elements.append(Paragraph(markets_text, styles['PhoenixBody']))

    elements.append(Paragraph(
        "With proxies in all configured markets, Phoenix provides access to a truly "
...
def create_technical_deep_dive(styles):
    ...
    elements.append(Paragraph(
        "This section provides a detailed walkthrough of the Phoenix codebase, explaining "
...
"""
    elements.append(Preformatted(proxy_code, styles['PhoenixCode']))
...
async def scrape_flights_from_market(origin, destination, date, market):
    ...
    async with async_playwright() as p:
        ...
        return flights
"""
    elements.append(Preformatted(scraper_code, styles['PhoenixCode']))
...
def find_matching_flight(base_flight, target_flights, target_currency):
    ...
    return None
"""
    elements.append(Preformatted(matching_code, styles['PhoenixCode']))
...
"""
    elements.append(Preformatted(arb_code, styles['PhoenixCode']))
...
"""
    elements.append(Preformatted(exclusive_code, styles['PhoenixCode']))
...
def create_business_model(styles):
    ...
    elements.append(Paragraph(
        "Phoenix operates on a success-fee model: we only charge when we save the customer money. "
    ...
    econ_data = [
        ["", "Without Phoenix", "With Phoenix", "Delta"],
...
def create_market_opportunity(styles):
    ...
    stats_table.setStyle(TableStyle([
        ('FONTSIZE', (0, 0), (-1, 0), 32),
        ('TEXTCOLOR', (0, 0), (-1, 0), PHOENIX_ORANGE),
        ('FONTNAME', (0, 0), (-1, 0), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 1), (-1, 1), 10),
    ...
    segments = [
        ...
        "use VPNs and incognito mode to find deals. Phoenix automates their workflow.",
    ...
    for segment in segments:
        elements.append(Paragraph(segment, styles['PhoenixBody']))

    elements.append(Paragraph("Competitive Landscape", styles['SubHeading']))

    comp_data = [
        ["Competitor", "Approach", "Phoenix Advantage"],
...
def create_conclusion(styles):
    ...
    elements.append(Paragraph(
        "Phoenix has identified and validated a systematic pricing inefficiency in the global "
    ...
    for takeaway in takeaways:
        elements.append(Paragraph(takeaway, styles['PhoenixBody']))
    ...
    # Call to action
    elements.append(Paragraph(
        "Phoenix is ready to bring price transparency to the flight booking industry.<br/>"
...
def generate_report():
    """Generate the complete Phoenix market presentation PDF."""

    # Output path
    output_path = "/Users/adramainjest/flightfinder2/PHOENIX_Market_Presentation.pdf"
    ...
    print(f"\n{'='*60}")
    print("PHOENIX Market Presentation Generated Successfully!")
    print(f"{'='*60}")
    ...
    print("  - The Solution: Phoenix Platform")
