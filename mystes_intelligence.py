"""
MYSTES Intelligence Engine

Connects Mystes's proprietary market data to AI search and commercial APIs.
...
class MystesIntelligence:
    """
    Intelligence layer bridging Mystes data to AI and commercial APIs.
    ...
    def build_ai_context(self, query, market=None):
        """
        Build rich context string from Mystes data for AI system prompts.

        This is the main entry point — called by ai_search.py to inject
        Mystes intelligence into every LLM query.
        ...
        if stats and stats.get("total_searches", 0) > 0:
            parts.append(
                f"Mystes platform: {stats['total_searches']:,} searches tracked, "
        ...
        if parts:
            return "MYSTES MARKET INTELLIGENCE (proprietary data):\n\n" + "\n\n".join(parts)
    ...
    def _fallback_market_context(self, market):
        """Minimal geographic context when no Mystes data exists."""
        try:
...
# Global instance
intelligence = MystesIntelligence()
